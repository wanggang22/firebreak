...
import { ACTION, ZERO_ADDR, type Address } from "./types.ts";
...
async function main() {
  ...
  // Solidity: rescue(address user, Plan{uint8,address,uint256,address,uint256,uint256,uint256})
  const sig = "rescue(address,(uint8,address,uint256,address,uint256,uint256,uint256))";
  const tuple = `(${p.action},${p.collateralToken},${p.collateralAmount},${p.rotateTo},${p.minSwapOut},${p.minSwapOut2},${p.topUpAmount})`;
  ...
  const out = {
    ...
    memo: decision.memo,
    call: { signature: sig, user, plan: tuple, contract: dep.mandate },
  };
