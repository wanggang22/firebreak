...
// DELEVERAGE only
{
  ...
  const t: Terms = { hfTriggerWad: trigger, maxSpendPerRescue: 10_000n * WAD, maxSlippageWad: WAD, minImprovementWad: 0n, keeperFee: 0n, allowedActions: ACTION.DELEVERAGE, reserve: 0n };
  const d = decide(s, t);
...
// TOPUP preferred when reserve available (cheapest)
{
  ...
  const s: Signals = { user: EURC, hf: hfOf(debt, cs), debt, collaterals: cs, quoteUsdcOut: quote };
  const t: Terms = { hfTriggerWad: trigger, maxSpendPerRescue: 10_000n * WAD, maxSlippageWad: WAD, minImprovementWad: 0n, keeperFee: 0n, allowedActions: ACTION.DELEVERAGE | ACTION.TOPUP, reserve: 500n * WAD };
  const d = decide(s, t);
...
// ROTATE restores health by parking into higher-LT asset
{
  const { cs, debt, quote, quoteToken } = scenario();
  const s: Signals = { user: EURC, hf: hfOf(debt, cs), debt, collaterals: cs, quoteUsdcOut: quote };
  const t: Terms = { hfTriggerWad: trigger, maxSpendPerRescue: 10_000n * WAD, maxSlippageWad: WAD, minImprovementWad: 0n, keeperFee: 0n, allowedActions: ACTION.ROTATE, reserve: 0n };
  const d = decide(s, t);
...
// spend cap blocks oversized plans
{
  const { cs, debt, quote } = scenario();
  const s: Signals = { user: EURC, hf: hfOf(debt, cs), debt, collaterals: cs, quoteUsdcOut: quote };
  const t: Terms = { hfTriggerWad: trigger, maxSpendPerRescue: 10n * WAD, maxSlippageWad: WAD, minImprovementWad: 0n, keeperFee: 0n, allowedActions: ACTION.DELEVERAGE, reserve: 0n };
  const d = decide(s, t);
...
// bug that once shipped: cheapest-by-cost picked a partial fix.
{
  ...
  const t: Terms = { hfTriggerWad: trigger, maxSpendPerRescue: 10_000n * WAD, maxSlippageWad: WAD, minImprovementWad: 0n, keeperFee: 0n, allowedActions: ACTION.TOPUP | ACTION.ROTATE, reserve: 50n * WAD };
  const d = decide(s, t);
